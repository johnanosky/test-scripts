...
case $1 in
	a)
		...
		if [ -z $TTY_DEV ]; then
			echo "No suitable interface found, please plug in device and try again."
			exit 1
		fi
		TTY_TEST=`ps -aux | grep minicom | grep ttyUSB$TTY_DEV | tail -n1 | awk -F " " '{print $2}'`
		if [ -z $TTY_TEST ]; then
			minicom -c on -D /dev/ttyUSB$TTY_DEV -8 -o -l -b $C_BAUD
		else echo "A minicom session with PID $TTY_TEST is already running on /dev/ttyUSB$TTY_DEV"
		fi
		;;
	imx7)
		...
		;;
	[0-9])
		TTY_TEST=`ps -aux | grep minicom | grep ttyUSB$1 | awk -F " " '{print $2}'`
		if [ -z $TTY_TEST ]; then
			minicom -c on -D /dev/ttyUSB$1 -8 -o -l -b $C_BAUD
		else echo "A minicom session with PID $TTY_TEST is already running on /dev/ttyUSB$1"
		fi
		;;
	*)
